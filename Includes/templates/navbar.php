
    
    <!-- START NAVBAR SECTION -->

    <header id="header" class="header-section">
        <div class="container">
            <nav class="navbar">
                <a href="index.php" class="navbar-brand">
                    <img src="Design/images/logo.png" alt="Restaurant Logo" style="width: 240px;">
                </a>
                <div class="d-flex menu-wrap align-items-center">
                    <div class="mainmenu" id="mainmenu">
                        <ul class="nav">
                            <li><a href="index.php#home">Trang chủ </a></li>
                            <li><a href="index.php#menus">Thực đơn</a></li>
                            <li><a href="index.php#gallery">Đặc trưng nổi bật</a></li>
<!--                            <li><a href="index.php#about">Giới thiệu</a></li>-->
                            <li><a href="index.php#contact">Liên hệ</a></li>
                        </ul>
                    </div>
                    <div class="header-btn" style="margin-left:10px">
                        <a href="table-reservation.php" target="_blank" class="menu-btn">Đặt bàn nào</a>
                    </div>
                </div>
            </nav>
        </div>
    </header>

	<div class="header-height" style="height: 120px;"></div>

    <!-- END NAVBAR SECTION -->